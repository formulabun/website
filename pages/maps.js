...
const FilterBar = (props) => {
  return (
    <Grid container> <Grid.Row verticalAlign="middle">
      <Grid.Column width={7}> <Input
        icon="search"
        fluid
      ...
      /> </Grid.Column>
      <Grid.Column width={5}>
        <Checkbox onClick={() => props.setShowHidden(!props.showHidden)} label="Show maps not in rotation."/>
      </Grid.Column>
      <Grid.Column floated="right" width={2}> <Button.Group floated="right">
        <Button active={props.showGrid} onClick={() => {props.setShowGrid(true)}} icon><Icon name="grid layout"/></Button>
        <Button active={!props.showGrid} onClick={() => {props.setShowGrid(false)}} icon><Icon name="th list"/></Button>
...
const MapsTable = (props) => {
  ...
  const displayData = props.data.filter((o) => showHidden || o.typeoflevel !== "singleplayer").filter((o) => searchMap(o, searchreg));
